...
JsiWorkletContext::JsiWorkletContext(const std::string& name) {
  // Initialize context
  initialize(name, JsiWorkletContext::getDefaultInstance()->_jsRuntime, JsiWorkletContext::getDefaultInstance()->_jsCallInvoker);
}

JsiWorkletContext::JsiWorkletContext(const std::string& name, std::function<void(std::function<void()>&&)> workletCallInvoker) {
  // Initialize context
  initialize(name, JsiWorkletContext::getDefaultInstance()->_jsRuntime, JsiWorkletContext::getDefaultInstance()->_jsCallInvoker,
             workletCallInvoker);
}

JsiWorkletContext::JsiWorkletContext(const std::string& name, jsi::Runtime* jsRuntime,
...
void JsiWorkletContext::initialize(const std::string& name, jsi::Runtime* jsRuntime,
                                   std::function<void(std::function<void()>&&)> jsCallInvoker,
                                   std::function<void(std::function<void()>&&)> workletCallInvoker) {
...
void JsiWorkletContext::initialize(const std::string& name, jsi::Runtime* jsRuntime,
                                   std::function<void(std::function<void()>&&)> jsCallInvoker) {
  // Create queue
  auto dispatchQueue = std::make_shared<DispatchQueue>(name + "_worklet_dispatch_queue_" + std::to_string(_contextId));

  // Initialize invoker
  initialize(name, jsRuntime, jsCallInvoker,
             [dispatchQueue = std::move(dispatchQueue)](std::function<void()>&& f) { dispatchQueue->dispatch(std::move(f)); });
}
...
jsi::Runtime& JsiWorkletContext::getWorkletRuntime() {
  if (!_workletRuntime) {
    ...
    // object.
    _workletRuntime->global().setProperty(*_workletRuntime, GlobalPropertyName, _workletRuntime->global());
...
void JsiWorkletContext::invokeOnJsThread(std::function<void(jsi::Runtime& runtime)>&& fp) {
  if (_jsCallInvoker == nullptr) {
    throw std::runtime_error("Expected Worklet context to have a JS call invoker.");
  }
...
void JsiWorkletContext::invokeOnWorkletThread(std::function<void(JsiWorkletContext* context, jsi::Runtime& runtime)>&& fp) {
  if (_workletCallInvoker == nullptr) {
    throw std::runtime_error("Expected Worklet context to have a worklet call invoker.");
  }
  _workletCallInvoker([fp = std::move(fp), weakSelf = weak_from_this()]() {
    auto self = weakSelf.lock();
    if (self) {
#ifdef ANDROID
      facebook::jni::ThreadScope::WithClassLoader([fp = std::move(fp), self]() { fp(self.get(), self->getWorkletRuntime()); });
#else
...
jsi::HostFunctionType JsiWorkletContext::createCallInContext(jsi::Runtime& runtime, const jsi::Value& maybeFunc, JsiWorkletContext* ctx) {

  // Ensure that we are passing a function as the param.
  if (!maybeFunc.isObject() || !maybeFunc.asObject(runtime).isFunction(runtime)) {
    throw jsi::JSError(runtime, "Parameter to callInContext is not a valid Javascript function.");
  }

  // Now we can save the function in a shared ptr to be able to keep it around
  auto func = std::make_shared<jsi::Function>(maybeFunc.asObject(runtime).asFunction(runtime));
  ...
  auto workletInvoker = JsiWorklet::isDecoratedAsWorklet(runtime, func) ? std::make_shared<WorkletInvoker>(runtime, maybeFunc) : nullptr;

  // Now return the caller function as a hostfunction type.
  return [workletInvoker, func, ctx](jsi::Runtime& runtime, const jsi::Value& thisValue, const jsi::Value* arguments,
    ...
    if (convention == CallingConvention::JsToJs || convention == CallingConvention::WithinCtx) {

      // Create promise
      auto promise =
          JsiPromiseWrapper::createPromiseWrapper(runtime, [ctx, convention, workletInvoker, callingCtx, thisWrapper, argsWrapper,
                                                            func](jsi::Runtime& runtime, std::shared_ptr<PromiseParameter> promise) {
            ...
            try {
              if (workletInvoker) {
                auto retVal = workletInvoker->call(runtime, unwrappedThis, ArgumentsWrapper::toArgs(args), argsWrapper.getCount());
                promise->resolve(runtime, retVal);
              } else {
                if (unwrappedThis.isObject()) {
                  auto retVal =
                      func->callWithThis(runtime, unwrappedThis.asObject(runtime), ArgumentsWrapper::toArgs(args), argsWrapper.getCount());
                  promise->resolve(runtime, retVal);
                } else {
                  auto retVal = func->call(runtime, ArgumentsWrapper::toArgs(args), argsWrapper.getCount());
                  promise->resolve(runtime, retVal);
                }
              }
            } catch (const jsi::JSError& err) {
              // TODO: Handle Stack!!
              promise->reject(runtime, jsi::String::createFromUtf8(runtime, err.getMessage()));
              ...
              if (a == b) {
                ...
                promise->reject(runtime, jsi::String::createFromUtf8(runtime, message));
              } else {
                // TODO: JsErrorWrapper reason("Unknown error in promise",
                // "[Uknown stack]");
                promise->reject(runtime, jsi::String::createFromUtf8(runtime, "Unknown error in promise"));
              }
            } catch (...) {
              // TODO: JsErrorWrapper reason("Unknown error in promise",
              // "[Uknown stack]");
    ...
    auto callIntoCorrectContext = [&runtime, convention, ctx](std::function<void(jsi::Runtime & runtime)>&& func) {
      switch (convention) {
        case CallingConvention::JsToCtx:
        case CallingConvention::CtxToCtx:
          ctx->invokeOnWorkletThread([func](JsiWorkletContext*, jsi::Runtime& rt) { func(rt); });
          break;
        ...
        default:
          ...
          throw jsi::JSError(runtime, "Should not be reached. Callback into context.");
      }
    };

    auto callback = [&runtime, convention, callingCtx, ctx](std::function<void(jsi::Runtime & rt)>&& func) {
    ...
    // correct contexts
    auto promise = JsiPromiseWrapper::createPromiseWrapper(runtime, [ctx, workletInvoker, convention, callingCtx, thisWrapper, argsWrapper,
                                                                     callIntoCorrectContext, callback, func](
                                                                        jsi::Runtime& runtime, std::shared_ptr<PromiseParameter> promise) {
      // Create callback wrapper
      callIntoCorrectContext([callback, workletInvoker, thisWrapper, argsWrapper, promise, func](jsi::Runtime& runtime) mutable {
        try {
          ...
          if (workletInvoker != nullptr) {
            result = workletInvoker->call(runtime, thisWrapper->unwrap(runtime), ArgumentsWrapper::toArgs(args), argsWrapper.getCount());
          } else {
            result = func->call(runtime, ArgumentsWrapper::toArgs(args), argsWrapper.getCount());
          }

          auto retVal = JsiWrapper::wrap(runtime, result);

          // Callback with the results
          callback([retVal, promise](jsi::Runtime& runtime) { promise->resolve(runtime, retVal->unwrap(runtime)); });
        } catch (const jsi::JSError& err) {
          ...
          if (a == b) {
            ...
            // TODO: Stack
            callback([err, promise](jsi::Runtime& runtime) { promise->reject(runtime, jsi::String::createFromUtf8(runtime, err.what())); });
          }
        } catch (...) {
...
jsi::HostFunctionType JsiWorkletContext::createInvoker(jsi::Runtime& runtime, const jsi::Value* maybeFunc) {
  auto rtPtr = &runtime;
  auto ctx = JsiWorkletContext::getCurrent(runtime);

  // Create host function
  return [rtPtr, ctx, func = std::make_shared<jsi::Function>(maybeFunc->asObject(runtime).asFunction(runtime))](
    ...
    if (ctx != nullptr) {
      ...
      ctx->invokeOnWorkletThread([argsWrapper, rtPtr, func = std::move(func)](JsiWorkletContext*, jsi::Runtime& runtime) mutable {
        assert(&runtime == rtPtr && "Expected same runtime ptr!");
        auto args = argsWrapper.getArguments(runtime);
        func->call(runtime, ArgumentsWrapper::toArgs(args), argsWrapper.getCount());
        func = nullptr;
      });
    } else {
...
JsiWorkletContext::CallingConvention JsiWorkletContext::getCallingConvention(JsiWorkletContext* fromContext, JsiWorkletContext* toContext) {
