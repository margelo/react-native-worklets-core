...
namespace RNWorklet {
    ...
    public:
        /**
         Constructs a shared value - which is a wrapped value that can be accessed in a
         thread safe across two javascript runtimes.
         */
        JsiSharedValue(const jsi::Value &value, JsiWorkletContext* context) :
            _valueWrapper(std::make_shared<JsiWrapper>(*context->getJsRuntime(), value)) {
            ...
            installFunction("addListener", JSI_FUNC_SIGNATURE {
                // This functionPtr should only be callable from the js runtime
                if(context->isWorkletRuntime(runtime)) {
                    jsi::detail::throwJSError(runtime,"addListener can only be called from the main Javascript code.");
                }
                ...
                if(arguments[0].isUndefined()|| arguments[0].isNull() ||
                    ...
                    jsi::detail::throwJSError(runtime,"addListener expects a function as its parameter.");
                }

                // Wrap the callback into a dispatcher with error handling. This callback will
                // always be called on the main js thread/runtime so we can just use values directly.
                ...
                auto thisValuePtr = std::make_shared<JsiWrapper>(runtime, jsi::Value::undefined());
                ...
                auto callback = std::make_shared<std::function<void()>>([context, dispatcher]() {
                ...
                jsi::PropNameID::forUtf8(runtime, "unsubscribe"), 0,
                    [=](jsi::Runtime & runtime,
