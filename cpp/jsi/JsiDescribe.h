   ...
   private:
    ...
    static std::string describeObject(jsi::Runtime& source, jsi::Object& object,
      ...
      if (object.isArray(source)) {
          ...
          std::string retVal = string_format(
              "%s%s: %i (Array size) = [\n",
              fromLevel(level).c_str(),
              name.c_str(),
              size);
          ...
          for (size_t i = 0; i < size; i++) {
              ...
              if (child.isObject() &&
                  child.asObject(source).isFunction(source)) {
                  jsi::detail::throwJSError(
                      source, "Functions in arrays not supported");
              }

            if(child.isObject()) {
              if(cache.count((size_t)&object) == 0) {
                // We don't have the value in the cache
                cache.emplace((size_t)&object, describe(
                    source,
                    child,
      ...
      for (size_t i = 0; i < names.length(source); i++) {
          auto propName =
              names.getValueAtIndex(source, i).asString(source).utf8(
                  source);
