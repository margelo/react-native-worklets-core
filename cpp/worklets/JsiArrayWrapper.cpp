...
JsiArrayWrapper::JsiArrayWrapper(
    jsi::Runtime &runtime, jsi::Array &array, JsiWrapper *parent,
  ...
  for (size_t i = 0; i < size; i++) {
    _array[i] = JsiWrapper::wrap(runtime, array.getValueAtIndex(runtime, i),
                                 parent, functionResolver, "");
  }
}

void JsiArrayWrapper::set(jsi::Runtime &rt, const jsi::PropNameID &name,
                          const jsi::Value &value) {
  // Do nada - an array set method is never called
}

jsi::Value JsiArrayWrapper::get(jsi::Runtime &runtime,
  ...
  if (nameStr == "length") {
    ...
    return JSIFN("push", {
      ...
      for (size_t i = 0; i < count; i++) {
        ...
        _array.push_back(JsiWrapper::wrap(runtime, arguments[i], _parent,
                                          _functionResolver, indexString));
      }
      if (_parent != nullptr) {
    ...
    return JSIFN("map", {
      ...
      for (size_t i = 0; i < _array.size(); i++) {
        jsi::Value retVal;
        if (thisArg.isUndefined()) {
          retVal = callbackFn.call(runtime,
                                   JsiWrapper::unwrap(runtime, _array.at(i)));
        } else {
