...
JsiWorkletContext::JsiWorkletContext(jsi::Runtime* jsRuntime,
    ...
    _workletRuntime->global().setProperty(*_workletRuntime, "_WORKLET", jsi::Value(true));
    ...
    _workletRuntime->global().setProperty(
            *_workletRuntime,
            "WorkletApi",
            jsi::Object::createFromHostObject(*_workletRuntime, std::move(workletApi)));
}
...
std::shared_ptr<JsiWrapper> JsiWorkletContext::copyToWorkletRuntime(const jsi::Value &value) {
    // Get runtimes
    jsi::Runtime* workletRuntime = &getWorkletRuntime();
    jsi::Runtime* jsRuntime = getJsRuntime();

    JsiFunctionResolver resolveFunction =
            [this, jsRuntime, workletRuntime]
            (const jsi::Value& candidate) -> jsi::HostFunctionType {
        ...
        if(JsiWorklet::isWorklet(*jsRuntime, candidate)) {
            ...
            auto name = candidate.asObject(*jsRuntime).getProperty(*jsRuntime, "name");
            std::string nameStr = "[unknown]";
            if(!name.isUndefined() && name.isString()) {
                nameStr = name.asString(*jsRuntime).utf8(*jsRuntime);
            }

            return [nameStr, jsRuntime] (
                  jsi::Runtime &rt, const jsi::Value &thisObj,
                  const jsi::Value *arguments, size_t count) -> jsi::Value {
                        jsi::detail::throwJSError(*jsRuntime,
                        std::string("Could not call function " + nameStr +
                                " on a worklet runtime because the function is not a worklet").c_str());
    ...
    // Create wrapper
    auto retVal = std::make_shared<JsiWrapper>(*jsRuntime,
                                               value,
...
jsi::Function JsiWorkletContext::getWorkletOrThrow(const jsi::Value &value) {
    ...
    if (!JsiWorklet::isWorklet(*jsRuntime, value)) {
        ...
        jsi::detail::throwJSError(
                *jsRuntime,
                std::string("Installing worklet failed.\nFunction " +
...
jsi::HostFunctionType JsiWorkletContext::getWorklet(const jsi::Function &worklet) {
    jsi::Runtime* jsRuntime = getJsRuntime();
    jsi::Runtime* workletRuntime = &getWorkletRuntime();
    ...
    if(!JsiWorklet::isWorklet(*jsRuntime, worklet)) {
        ...
        jsi::detail::throwJSError(
                *jsRuntime,
                std::string("Installing worklet failed.\nFunction " +
                            nameStr + " is not a worklet.").c_str());
    }
    ...
    if (getWorkletsCache().count(workletHash) == 0) {
        ...
        auto code =
                worklet.getProperty(*jsRuntime, WorkletAsStringPropertyName)
                        .asString(*jsRuntime)
                        .utf8(*jsRuntime);

        // Evaluate and store function in cache
        auto eval = getWorkletRuntime().global().getPropertyAsFunction(getWorkletRuntime(), "eval");
        _workletsCache.emplace(workletHash, JsiWorkletInfo {
            std::make_shared<jsi::Function>(eval
                .call(*workletRuntime, ("(" + code + ")").c_str())
    ...
    return [workletRuntime, installedWorklet, closureWrapper, this](
            jsi::Runtime &rt,
            const jsi::Value &thisObj,
        ...
        if (&rt != workletRuntime) {
            jsi::detail::throwJSError(
                    *workletRuntime,
                    "Worklet called on another runtime than the one it was installed into.");
        }

        // Now we are ready and set up to run this one. Start by storing the
        // previous jsThis object
        jsi::Value oldJSThis =
                rt.global().getProperty(rt, WorkletJsThisName);
        ...
        try {
            if (thisObj.isUndefined() || thisObj.isNull()) {
                ...
                retVal = installedWorklet.worklet->call(
                        rt, arguments, count);
            } else {
            ...
            // Rethrow
            jsi::Value location =
                    jsThis.getProperty(rt, WorkletLocationPropertyName);
...
jsi::HostFunctionType JsiWorkletContext::getWorklet(const jsi::Value &function) {
    // Get the function / worklet
    auto worklet = getWorkletOrThrow(function);
    return getWorklet(worklet);
}

std::vector<JsiSharedValue*> JsiWorkletContext::getSharedValues(const jsi::Function &worklet) {
    jsi::Runtime* jsRuntime = getJsRuntime();
    ...
    JsiVisitor::visit(*getJsRuntime(), closure,
        ...
        if(value.isObject() && value.asObject(runtime).isHostObject(runtime)) {
            ...
            JsiSharedValue* wrapper = dynamic_cast<JsiSharedValue*>(hostObject.get());
            if(wrapper != nullptr) {
